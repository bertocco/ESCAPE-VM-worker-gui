...
def create_fedcloud_VM(token, site, vo, flavour_selected, image_selected, network_selected, ssh_key_name, vm_name):
    print("Creating VM ...")
    print("Network is :" + network_selected)
...
def create_public_vm(token, site, vo, flavour_selected, image_selected, network_selected, ssh_key_name, vm_name, public_network):
    err_code = create_fedcloud_VM(token, site, vo, flavour_selected, image_selected, network_selected, ssh_key_name, vm_name)
    if(err_code != 0):
        return  err_code, "ERROR creating Virtual Machine"
    pub_IP = get_a_public_IP(token, site, vo, public_network)
    if(pub_IP != 0):
        err_cod = assign_public_IP_to_VM(token, site, vo, pub_IP, vm_name)
        if(err_cod != 0):
            delete_fedcloud_VM(token, site, vo, vm_name, json_output=False)
            return err_code, "ERROR assigning public IP"
    else:
        delete_fedcloud_VM(token, site, vo, vm_name, json_output=False)
        return err_cod, "ERROR getting a public IP"
    return 0, pub_IP
